...
type PaymentsQuery struct {
	...
	// NumMaxPayments is the maximal number of payments returned in the
	// payments query.
	NumMaxPayments uint64
	...
	// If IncludeIncomplete is true, then return payments that have
	// not yet fully completed. This means that pending payments,
	// as well as failed payments will show up if this
...
func (db *DB) QueryPayments(query PaymentsQuery) (PaymentsQuerySlice, error) {
	...
	var count uint64
	for ; count < query.NumMaxPayments && ok; i, ok = nextIndex(
		...
		// Update first and last index offsets.
		if query.Reversed {
