...
// and integers which represent the index of the first and last item returned in
// the series of payments. These integers allow callers to resume their query in
// the event that the query's response exceeds the max number of returnable
// events.
...
func (db *DB) QueryPayments(query PaymentsQuery) (PaymentsQuerySlice, error) {
	...
	// we set our limit to our highest sequence number + 1, to also include
	// the highest sequence number since the limit is exclusive.
	if query.Reversed && indexExclusiveLimit == 0 {
		indexExclusiveLimit = allPayments[len(allPayments)-1].SequenceNum + 1
	}
