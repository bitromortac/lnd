...
The `peer` and `brontide` subsystems handle the encrypted transport (BOLT 8)
for the [Lnd Architecture](202603181000-Lnd-Architecture.md). They manage the
...
interfaces. This allows higher-level subsystems, such as the
[HTLC Switch](202603181002-htlc-switch-routing.md), to transmit serialized
messages (e.g., HTLC adds, settles, or fails) or custom feature sets
transparently without needing to understand the underlying cryptographic
transport protocols or reconnect logic.
