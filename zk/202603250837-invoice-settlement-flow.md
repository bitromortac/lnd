...
value and CLTV constraints, the node accepts it. However, the transaction is
not finalized until the node reveals the cryptographic preimage corresponding to
the invoice's payment hash.

The settlement process requires the node to first verify that the invoice is
not already settled or canceled, respecting the [invoice state
...
Simultaneously, the settlement flow updates the [invoice database
storage](202603250839-invoice-database-storage.md) to record the terminal
`Settled` state along with the amount paid and the settlement time. Any external
clients monitoring the invoice via [invoice
subscriptions](202603250840-invoice-subscriptions.md) receive a notification of
the transition, allowing the application layer to deliver the purchased goods
or services.
