...
type ForwardHtlcInterceptRequest struct {
	...
	// The requested outgoing next hop for a blinded route that identifies it by
	// node id (next_node_id) rather than by channel: the next hop's compressed
	// public key. It is empty for the common channel-id case. When set,
	// outgoing_requested_chan_id carries a reserved sentinel value and lnd
	// selects an actual outgoing channel to this peer via non-strict
	// forwarding.
