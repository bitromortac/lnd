...
func BuildInvoiceRequest(offer *bolt12.Offer, opts ...RequestOption) (
	*bolt12.InvoiceRequest, *btcec.PrivateKey, error) {
	...
	// Encode runs the writer validation, so a malformed request fails
	// here rather than after the payer key has signed it. The bytes are
	// not needed: SignInvoiceRequest derives the Merkle root from
	// AllRecords.
...
// original invoice request and the path the payer actually used. It performs
// structural validation, signature verification, byte-for-byte field matching,
// expiry, and the binding of invoice_node_id to the node the payer expected
// to answer. activeChain is the genesis hash the sender is willing to settle
// on; it gates the spec invreq_chain reader rule. now is the clock the expiry
// check reads, which the codec takes from its caller.
//
// finalBlindedNodeID is the final blinded node on the path the payer used. It
// is only consulted for an offer that carried no issuer id, and for an
// offerless request, where the caller passes the node it sent to.
func ValidateInvoiceReply(inv *bolt12.Invoice, req *bolt12.InvoiceRequest,
	finalBlindedNodeID *btcec.PublicKey, activeChain [32]byte,
	now time.Time) error {

	// An offer that named its issuer binds the invoice to that key, so it
	// takes precedence over the path the payer happened to use.
	expectedNodeID := finalBlindedNodeID
	req.OfferIssuerID.WhenSome(
		func(r tlv.RecordT[tlv.TlvType22, *btcec.PublicKey]) {
			expectedNodeID = r.Val
		},
	)
