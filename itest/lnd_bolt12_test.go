...
func testBolt12InvoiceRequest(ht *lntest.HarnessTest) {
	...
	// Encode (validates the writer rules) → sign → re-encode.
	...
	case msg := <-bobMessages:
		...
		// The string encoder requires a populated signature and
		// verifies it against invoice_node_id.
		_, encErr := bolt12.EncodeInvoiceString(invoice)
		require.NoError(ht, encErr, "verify invoice sig")
