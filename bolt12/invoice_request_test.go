...
// TestDecodeInvoiceRequestVector decodes the invoice_request from the signature
// test vector and verifies key fields.
func TestDecodeInvoiceRequestVector(t *testing.T) {
	t.Parallel()

	// From signature-test.json: the invoice_request bolt12 string.
	...
	// tree — every wire TLV must be reachable through AllRecords for
	...
	// reordering, dropped TLV, or non-canonical integer would invalidate it.
